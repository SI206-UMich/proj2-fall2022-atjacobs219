...
.
    ...
    num_beds = soup.find_all("li", class_ = 'l7n4lsf')[1]
    num_beds_2 = num_beds.find_all("span")[2]
    num_of_beds = num_beds_2.text
    if "studio" in num_of_beds.lower():
        final_bed_num = 1
    else:
        final_bed_num = int(num_of_beds.split()[0])
...
def get_detailed_listing_database(html_file):
    ...
    for item in first_function:
        #print(first_function[-1])
        listing_ids = item[-1]
        #print(listing_ids)
        second_function = get_listing_information(listing_ids)
        tup = (item + second_function)
        #print(tup)
        compiled_list.append(tup)
    #print(compiled_list)
...
def check_policy_numbers(data):
    ...
    """
    policy = []
    regex = r'^STR-000[0-9]{4}$|^20[0-9]{2}-00[0-9]{4}STR$'
    for line in data:
        x = line[3]
        #print(x)
        found_nums = re.findall(regex, x)
        if x == "Pending":
            continue
        if x == "Exempt":
            continue
        if len(found_nums) == 0:
            policy.append(line[2])
...
class TestCases(unittest.TestCase):
    ...
    def test_get_listing_information(self):
        ...
        self.assertEqual(listing_informations[3][-1], 1)
